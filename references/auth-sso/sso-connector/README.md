# 模式 C：V8 → 三方单点 — SPI 接口完整定义

> **Evidence: FACT ✅** — 接口签名、枚举、注解、工具类均来自 `cip-connector-api` CFR 反编译。
> 完整契约包：`../shared/cip-connector-api-contract.md`
...
| 方法 | 参数 | 返回 | 语义 |
|------|------|------|------|
| getName | 无 | `String` | 唯一标识（全英文），类名 = name + "SsoServiceImpl" |
| getTypeCaption | 无 | `String` | 中文名称 |
| login | `String, String, Map, SsoClientTypeEnum, Map` | `String` | 核心：构建单点登录跳转地址 |
| check | `String config` | void | 校验前端配置参数 |
| getPageJson | 无 | `String` | 前端配置页面 JSON |
| getSortNo | 无 | `Integer` | 排序号 |
| needUserBind | 无 | `boolean` | 是否需要用户绑定（true 时 login 的 userMap 才有数据） |

## 代码骨架

```java
package com.seeyon.extend.spi.sso;
...
public class {Prefix}SsoServiceImpl implements SsoService {
    ...
    public String login(String url, String json, Map<String, Object> userMap,
        ...
        try {
            ...
            log.info("{project_name_cn} SSO, 用户: {}, URL: {}", loginName, url);

            // 4. 构建单点登录地址
            // 根据三方系统的认证方式拼接参数
            String ssoUrl = buildSsoUrl(url, loginName, userCode, ssoConfig);

            log.info("{project_name_cn} SSO 最终地址: {}", ssoUrl);
            return ssoUrl;

        } catch (Exception e) {
            log.error("{project_name_cn} SSO 失败", e);
    ...
    public boolean needUserBind() {
        return {need_user_bind}; // true 时 userMap 才有用户数据
    }
...
```properties
com.seeyon.cip.connector.api.sso.SsoService=com.seeyon.extend.authsso.connector.YourSsoServiceImpl
```

注意：SsoService 无注解路由，靠类名匹配（`getName() + "SsoServiceImpl"`）。
...
scope 值：
- 如果是 cip-connector 通用：`"cip-connector"`
- 如果是特定应用：`"{应用编码}"`（如 `"dongfangtong9044958822953602326"`）
