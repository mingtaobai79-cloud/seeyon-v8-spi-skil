---
title: "单元测试"
source: "https://www.yuque.com/seeyonkk/v8/wvb9zfc2wahwkgof"
...
8
【强制】单元测试类以Test.java结尾，测试方法以test*开头。

说明：一个测试类只对应一个被测试类，如测试类 DemoTest.java 测试 Demo.java类；一个测试方法只测试一个方法，如测试方法testFoo() 测试 foo()方法。

9
【强制】单元测试的目标：语句覆盖率达到99%；分支覆盖率达到100%。

说明：在AppService层和Dao层，都应该进行单元测试（Dao层只需要做自定义SQL的单元测试）。
